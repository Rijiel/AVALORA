...
public static class UserHelper
{
	...
	public static async Task<IdentityUser?> GetIdentityUserAsync(IHttpContextAccessor accessor, UserManager<IdentityUser> userManager)
	{
...
/*
	...
	public static string? GetCurrentUserRole(IHttpContextAccessor accessor)
	{
		if (accessor.HttpContext?.User?.Identity?.IsAuthenticated ?? false)
		{
			if (accessor.HttpContext.User.Identity is ClaimsIdentity claimsIdentity)
	...
	public static async Task<string?> GetRoleByUserIdAsync(string? userId, UserManager<IdentityUser> userManager)
	{
		if (userId != null)
		{
	...
	/// <param name="userManager">The UserManager instance used to manage IdentityUser instances.</param>
	/// <returns>A Task that represents the asynchronous operation, containing the user's role if found, or null otherwise.</returns>
	public static async Task<string?> GetUserRole(string? userId, UserManager<IdentityUser> userManager)
	{
		if (userId != null)
		{
			var user = await userManager.FindByIdAsync(userId);
